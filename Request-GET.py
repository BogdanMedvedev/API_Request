...
def get_url() -> str:
    """Данная фунция по очереди возвращает каждую ссылку с товаром
...
def array() -> tuple:
    """Данная фунция обрабатывает каждую ссылку из функции get_url и возвращает
...
def uploading() -> None:
    """Данная функция записывает в csv файл наименование, цену и описание товара
    и выгружает картинку каждого товара в папку images. Папка должна быть в корне проекта"""
    for i in array():
